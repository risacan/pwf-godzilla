...
static void main_window_load(Window *window) {
  ...
  s_background_layer = bitmap_layer_create(
   GRect(PBL_IF_ROUND_ELSE(-55, -65), PBL_IF_ROUND_ELSE(-40, -40), 280, 250)
  );
  ...
  s_time_layer = text_layer_create(
    GRect(0, PBL_IF_ROUND_ELSE(20, 10), bounds.size.w, 50));
  ...
  // GFontを作成
  s_time_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_JANYK_30));
